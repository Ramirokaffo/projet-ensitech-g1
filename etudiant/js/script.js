...
class Student {
    constructor(matricule, first_name, last_name, email, 
        phone_number, address, gender, born_date) {
    ...
    static getAllStudent() {
      ...
      if (listeStudentString) {
        ...
        return listeStudent.map((student) => {
          ...
          return new Student(oneStudenteList[0], 
            oneStudenteList[1],
            oneStudenteList[2],
            oneStudenteList[3],
            oneStudenteList[4],
    ...
    static searchStudent(query) {
      ...
      if (listeStudentString) {
        ...
        listeStudent = listeStudent.filter((e) => e.toLowerCase().includes(query.toLowerCase()));
    ...
    static addStudent(listStudent) {
      ...
      const listCombine = maListe.map((e) => [e.matricule, e.first_name, e.last_name, e.email, e.phone_number, 
        e.address, e.gender, e.born_date].join("^*^*^"));
    ...
    static setStudent(listStudent) {
      ...
      const listCombine = listStudent.map((e) => [e.matricule, e.first_name, e.last_name, e.email, e.phone_number, 
        e.address, e.gender, e.born_date].join("^*^*^"));
      localStorage.setItem("student", listCombine.join("&%&%@"));
    }
...
if (searchQuery) {
  ...
  document.getElementById("update_student_link").onclick = (e) => {
    const selectedInput = document.querySelector(".form-check-input:checked");
    if (selectedInput) {
      localStorage.setItem("student_to_modify", selectedInput.getAttribute("student_matricule"));
    } else {
      e.preventDefault();
    }
  };
